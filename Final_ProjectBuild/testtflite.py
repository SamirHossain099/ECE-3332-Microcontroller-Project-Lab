...
def preprocess_image(image, input_shape):
    ...
    image = (image - 127.5) / 127.5  # Normalize if needed
    return image
...
def main():
    ...
    interpreter = tf.lite.Interpreter(model_path="Final_Project_Build_JetsonOrinNano/pytorch_model/model.tflite")
    ...
    try:
        while True:
            ...
            print(f"Output data 1: {output_data_1}")
            print(f"Output data 2: {output_data_2}")

            detected_object = False
            for i in range(output_data_1.shape[1]):
                result = output_data_1[0][i]
                # Assuming the result contains [x1, y1, x2, y2, confidence, class_id]
